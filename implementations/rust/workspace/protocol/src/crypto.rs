...
/// Fixed size of ballot serialization for the new ballot structure (u8 + u128)
/// The ballot structure with VSerializable always serializes to exactly 25 bytes
pub const PADDED_BYTE_ARRAY_SIZE: usize = 25;
...
/// Represents a partially decrypted component of a ciphertext, along with its
/// proof of correctness. This is generated by a trustee during the distributed
/// decryption phase.
#[derive(Debug, Clone, PartialEq, VSerializable)]
pub struct PartialDecryption {
    /// The partially decrypted value as a group element.
    pub decrypted_component: SelectionElement,
    /// The zero-knowledge proof demonstrating the correctness of the partial decryption.
...
pub fn encrypt_ballot(
    ...
    let mut encryption_context = b"ballot_encryption_".to_vec();
    encryption_context.extend_from_slice(election_hash);
...
pub fn decrypt_ballot(
    ...
    let mut decryption_context = b"ballot_encryption_".to_vec();
    decryption_context.extend_from_slice(election_hash);
...
pub fn verify_ciphertext_proof(
    ...
    let mut verification_context = b"ballot_encryption_".to_vec();
    verification_context.extend_from_slice(election_hash);
...
pub fn encrypt_test_ballot(
    ...
    encrypt_ballot(ballot, 1, &election_keypair.pkey, election_hash).unwrap()
}
...
mod tests {
    ...
    fn test_encrypt_ballot_function() {
        ...
        let ballot_style: BallotStyle = 5;
        ...
        let (ballot_cryptogram, randomizers) =
            encrypt_ballot(ballot, ballot_style, &election_keypair.pkey, &election_hash).unwrap();
